...
# テキスト入力
name = st.text_input('あなたの名前を入力してください', key='name_input')
if name:
    st.write(f'こんにちは、{name}さん！')

# 数値入力
age = st.number_input('あなたの年齢を入力してください', min_value=0, max_value=120,
                      value=20, step=1, key='age_input')
st.write(f'あなたは{age}歳です。')

# 日付入力
date = st.date_input('日付を選択してください', key='date_input')
...
data = {
    '月': ['1月', '2月', '3月', '4月', '5月', '6月'],
    '売上': [100, 120, 150, 130, 160, 180],
    '費用': [80, 90, 100, 110, 120, 130]
}
df = pd.DataFrame(data)
